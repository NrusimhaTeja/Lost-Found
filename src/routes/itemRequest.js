...
router.post("/request/send/:type/:itemId", userAuth, async (req, res) => {
  try {
    ...
    if (
      (itemDetails.status === "lost" && requestType === "claim") ||
      (itemDetails.status !== "lost" && requestType === "return") ||
      itemDetails.currentHolder === req.user._id
    ) {
      return res.status(400).json({ error: "bad request" });
    }
    const AlreadyRequested = await ItemRequest.findOne({
      itemId,
    ...
    let request;
    if (itemDetails.status === "lost") {
      request = ItemRequest({
        itemId,
        requestType,
        requestedBy: req.user._id,
        requestedTo: itemDetails.owner,
      });
    } else {
      request = ItemRequest({
        itemId,
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        !request.requestedTo.equals(req.user._id)
      ) {
        return res.json({ message: "Invalid operation" });
      }
      if (status === "rejected") {
        ...
        item.currentHolder =
          request.requestType === "claim" ? requestUserId : item.currentHolder;
        item.status = "claimed";
        await request.save();
        await item.save();
...
router.get("/request/send", userAuth, async (req, res) => {
  try {
    ...
    const requestSendList = await ItemRequest.find({
      requestedBy: userId,
      status: "pending",
    })
...
router.get("/request/receive", userAuth, async (req, res) => {
  try {
    const userId = req.user._id;
    const requestReceivedList = await ItemRequest.find({
      requestedTo: userId,
      status: "pending",
    })
